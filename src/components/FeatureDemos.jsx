...
export default function FeatureDemos() {
  ...
  const openDemo = (demo) => {
    ...
    if(demo.type === "conveyor") demo = { ...demo, items: [{name:"Item 1", category:""}, {name:"Item 2", category:""}, {name:"Item 3", category:""}] };
  ...
  const getPortfolioReply = (message) => {
    ...
    for (let qa of portfolioQA) {
      if (msg === qa.question.toLowerCase()) return qa.answer;
    }


    for (let qa of portfolioQA) {
      if (msg.includes(qa.question.toLowerCase().split(" ")[0])) return qa.answer;
    }
  ...
  const startMachineProducts = (machineName, idx) => {
    ...
    setProductIndex(0);
    setActiveMachine({name:machineName, idx});
  };
      ...
      <div className="max-w-6xl mx-auto px-6 space-y-12">
        ...
        {activeDemo && (
          <div className="fixed inset-0 bg-black bg-opacity-60 flex justify-center items-center z-50 p-4">
            <div className="bg-gray-900 rounded-2xl p-6 w-full max-w-lg shadow-2xl relative border border-gray-700">
                ...
                <>
                  ...
                  {qr && (
                    <div className="text-center mt-5">
                      ...
                      <a
                        href={qr}
                        download={`qr_demo_${activeDemo.id}.png`}
                        className="inline-block mt-2 text-indigo-400 underline hover:text-indigo-200 transition text-sm"
                      >
                        Download QR
                      </a>
                    </div>
              ...
              {activeDemo.type === "chat" && (
                <div className="flex flex-col h-96">
                  ...
                  <div className="relative">
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        if (val.trim() === "") setSuggestedQuestions(portfolioQA);
                        else setSuggestedQuestions(portfolioQA.filter(qa => qa.question.toLowerCase().includes(val.toLowerCase())));
                      }}
                      ...
                      <div className="absolute z-10 bg-gray-700 mt-1 w-full rounded-lg shadow-lg max-h-40 overflow-y-auto">
                        {suggestedQuestions.map((qa, idx) => (
                          <div
                            ...
                            className="px-4 py-2 hover:bg-gray-600 cursor-pointer text-white rounded-t-lg"
                          >
                            {qa.question}
                          </div>
                        ))}
                  ...
                  <button
                    onClick={() => sendMessage()}
                    className="mt-2 w-full px-6 py-2 bg-green-500 hover:bg-green-600 rounded-xl text-white font-semibold shadow transform hover:scale-105 transition"
                  >
                    Send
                  </button>
                </div>
              ...
              {activeDemo.type === "todo" && (
                <>
                  <div className="flex gap-2 mb-3">
                    ...
                    <button
                      onClick={addTask}
                      className="px-6 bg-green-500 hover:bg-green-600 rounded-xl text-white font-semibold shadow transform hover:scale-105 transition"
                    >
                      Add
                    </button>
                  </div>
  ...
  <div className="flex flex-col space-y-6">
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
      {activeDemo.machines.map((machine, idx) => {
        ...
        return (
          <div
            key={idx}
            className="p-5 rounded-2xl bg-gray-900 flex flex-col items-center shadow-lg hover:shadow-xl transition"
          >
            {/* Trạng thái máy */}
            <div className={`w-14 h-14 rounded-full flex items-center justify-center ${colors[status]} text-black font-bold mb-3`}>
              {status[0]}
            </div>
            ...
            <div className="flex space-x-3">
              <button
                className="px-3 py-1 bg-indigo-500 rounded-lg text-white font-medium hover:bg-indigo-600 transition transform hover:scale-105"
                onClick={() => {
              ...
              <button
                className="px-3 py-1 bg-purple-500 rounded-lg text-white font-medium hover:bg-purple-600 transition transform hover:scale-105"
                onClick={() => startMachineProducts(machine, idx)}
              >
                View Products
              </button>
            </div>
    ...
    {activeMachine && (
      <div className="mt-6 p-5 bg-gray-900 rounded-2xl shadow-lg">
        <h4 className="text-white font-bold text-lg mb-3">{activeMachine.name} Conveyor</h4>
        <div className="flex space-x-4 overflow-x-auto p-3 scrollbar-thin scrollbar-thumb-indigo-500 scrollbar-track-gray-700">
          {products.map((p, i) => (
            <div
              key={i}
              className={`flex-none w-28 h-28 flex items-center justify-center rounded-xl text-white font-semibold text-center
